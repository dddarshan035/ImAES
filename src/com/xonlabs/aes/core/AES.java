...
//		System.out.println("Time taken : "+(en-st));
		...
		long st = System.currentTimeMillis();
		decryptFile(filepath+ ".enc", secret);
		long en = System.currentTimeMillis();
		System.out.println("Time taken : "+(en-st));
...
/*
	...
	public static void main(String arg[]) throws IOException {
		...
		// for (int i = 0; i <= n; i++) {
		// encryptFile(filepath + ".part" + i, secret);
		// }
		// for (int i = 0; i <= 5; i++) {
		// decryptFile(filepath + ".part" + i + ".enc", secret);
		// }
		...
		// Encryption
		// for (int i = 0; i <= n; i++) {
		// EncryptionThread et = new EncryptionThread(filepath + ".part" + i,
		// secret);
		// encThreads.add(et);
		// }
		// try {
		...
		int n = 5;
		for (int i = 0; i <= n; i++) {
			DecryptionThread dt = new DecryptionThread(filepath + ".part" + i + ".enc", secret);
			decThreads.add(dt);
		}
		try {
		...
		FileUtil.merge(fileList, "C:\\AES\\data\\merged.txt");
